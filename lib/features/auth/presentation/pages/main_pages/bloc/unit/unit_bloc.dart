...
class UnitBloc extends Bloc<UnitEvent,UnitState>{
  ...
  UnitBloc({required this.getUnitsUsecase,required this.addUnitUsecase,required this.updateUnitUsecase,required this.deleteUnitUsecase}):super(UnitLoadingState()){
 ...
 Future<void> addingUnit(UnitAddingEvent event,Emitter<UnitState>emit)async{
  await addUnitUsecase(event.unitsEntity);
  add(UnitLoadingEvent());
 }
  
  Future <void>updatingUnit(UnitUpdatingEvent event,Emitter<UnitState>emit)async{
   await updateUnitUsecase(event.unitsEntity);
   add(UnitLoadingEvent());
  }

  Future<void>deleteUnit(UnitDeletingEvent event,Emitter<UnitState>emit)async{
